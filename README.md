...
The work aims to see the different behaviors between KNN and
...
-   *Train*: the sample of data used to fit the model. The classifier sees and learns from this data.

-   *Validation*: the sample of data used to provide an evaluation of a
    model fit on the training dataset while tuning model hyperparameters. So the validation set in a way affects a model, but indirectly.

-   *Test*: The sample of data used to provide an unbiased evaluation of a final model fit on the training dataset. It is only used
    when the model is completely trained.

The first set is composed of 50\% of the data, the second one by
...
point and the elements contained in the train set so if one of the features
...
[StandardScaler](https://scikit-learn.org/stable/modules/generated/sklearn.preprocessing.StandardScaler.html) was used.

Multiple models have been built using *K* between 1 and 7 using the
[KNeighborsClassifier](https://scikit-learn.org/stable/modules/generated/sklearn.neighbors.KNeighborsClassifier.html) to which the train set was
...
The decision boundaries obtained for *C = 0.001* is since
...
On the other hand, when it was assigned larger values to *C* the model tries
...
classification problem then the data was projected into a higher
...
![Decision boundaries when the kernel is RBF and
...
![Decision boundaries when the kernel is RBF and $C$=
...
was not taken, as the previous experiments show that with this value 
the worse model was obtained compared to the other values.
...
the test set of the classifier, trained with these values, is 81.48\%.
The model is more precise than the previous one because both *gamma*
and *C* have been tuned at the same time.

After the merge of training and validation split, it was done a grid search
...
test set of the classifier, trained with these values, is 87.04\%.
...
which is a filter type method that assigns a score to the features. This
score goes between -1 and 1, where a greater weight indicates a greater
relevance of this feature. The score was calculated using only the train and
validation set. Below are shown the scores for each feature.
...
9.  non flavonoid phenols 0.061

10. the alkalinity of ash 0.054
...
magnesium*) and (*od280/od315 of diluted wine, flavonoids*).
...
In machine learning and statistics, classification is the problem of identifying to which of a set of categories a new observation
belongs, based on a training set of data containing observations
...
Classification is done by a majority vote of *K* nearest neighbors. The
basic nearest neighbors classification uses uniform weights, under some
circumstances, it is better to weigh the neighbors such that nearer
...
model. Each dataset has it is own requirements but usually the value of *K* is odd when the number of classes is two, to avoid the same
...
the distance that could be used for all types of datasets as the free lunch
theorem states.

There is a possibility, using different distance metrics, that we might
...
the algorithm outputs an optimal hyperplane that categorizes new
...
hyperplanes that separate the two classes of data. The region bounded by these two hyperplanes is called the “margin”, and the maximum-margin
